...
static double K_y = 0.0;
static Matriz H_mat;
static Matriz A_cons;
static Matriz B_cons;
static Matriz f_mat;
static Matriz Delta_U_opt;
static Matriz Phi_T_F;
static Matriz Phi_T_Rs;
		 ...
		 void ** reserved_UserData, int reserved_ThreadIndex, void * reserved_AppPtr)
{
    ...
    double buff_F_xki[BUFF]; Matriz M_F_xki = matriz(buff_F_xki, Nc, 1);
    double buff_Rs_r[BUFF];  Matriz M_Rs_r  = matriz(buff_Rs_r, Nc, 1);
    ...
    mat_mult(&Phi_T_F, &x_ki, &M_F_xki);  // Phi_T_F * x_ki
    mult(&Phi_T_Rs, &M_Rs_r, r_ki);       // Phi_T_Rs * r_ki
    sub(&M_F_xki, &M_Rs_r, &f_mat);       // Subtrai e joga na variável global f_mat
    
    for(i = 0; i < Nc; i++) {
        B_cons.matriz[0 * Nc + i] = 0.2;               // du_max
        B_cons.matriz[1 * Nc + i] = 0.2;               // -du_min (0.2)
        B_cons.matriz[2 * Nc + i] = cic - u_anterior;  // u_max - u(k-1)
        B_cons.matriz[3 * Nc + i] = u_anterior;        // -u_min + u(k-1) (u_min é 0)
    }

    qp_hild(&H_mat, &f_mat, &A_cons, &B_cons, &Delta_U_opt);
	
    du_calc = u_anterior + Delta_U_opt.ret(&Delta_U_opt, 0, 0);
    if(du_calc > cic) { du_calc = cic; } 
    else if(du_calc < 0) { du_calc = 0; } 
...
{
    // =========================================================================
    // 1. DECLARAÇÕES NO TOPO ABSOLUTO (C89)
    // =========================================================================
    int i, j;
    double I_val, C2_val;
    
    // Declaração de Matrizes locais para evitar erro de C89
    Matriz Phi_T;
    Matriz Phi_T_Phi;
    ...
    static double phi_buff[BUFF];
    static double f_buff[BUFF];
    static double h_mat_buff[BUFF];
    static double a_cons_buff[BUFF];
    static double b_cons_buff[BUFF];
    static double phi_t_buff[BUFF];
    static double phi_t_phi_buff[BUFF];
    
    // Buffers para as Matrizes do Gradiente (Adicionado)
    static double phi_t_f_buff[BUFF];
    static double phi_t_rs_buff[BUFF];

    static int N_lag;
    static int n_estados;

	static double pt_f_buff[BUFF], pt_rs_buff[BUFF], f_buff2[BUFF];
    static double h_buff[BUFF], du_opt_buff[BUFF];
    ...
    Delta_T = 1.0/300e3; // (Corrigido) Atribuição da frequência de amostragem
    Ts = Delta_T;
    n_estados = 5;

    // =========================================================================
    // 3. INICIALIZAÇÃO E DISCRETIZAÇÃO
    // =========================================================================
    x_anterior = matriz(x_ant, n_estados, 1);
    x_ki       = matriz(xki, n_estados, 1);
    ...
    Rs_barra  = matriz(rs_barra, Np, 1);
    for (i = 0; i < Np; i++) {
        Rs_barra.matriz[i] = 1.0;
    }
    
    R_barra1 = matriz(r_barra1, LC_MAX, LC_MAX); 
    ident(&R_barra1, Nc);
    R_barra = matriz(r_barra, LC_MAX, LC_MAX);
    mult(&R_barra1, &R_barra, rw); 

    Delta_U = matriz(deltau, LC_MAX, LC_MAX);
    Phi = matriz(phi_buff, LC_MAX, LC_MAX);
    F   = matriz(f_buff, LC_MAX, LC_MAX);
    func_Phi(&A, &B, &C, Np, Nc, &Phi);
    func_F(&A, &C, &F, Np);

    // =========================================================================
    // 4. PREPARAÇÃO DO OTIMIZADOR DE HILDRETH (Hessiana, Limites e Gradiente)
    // =========================================================================
   Phi_T_F     = matriz(pt_f_buff, Nc, 5); // O número de estados é 5!
    Phi_T_Rs    = matriz(pt_rs_buff, Nc, 1);
    f_mat       = matriz(f_buff2, Nc, 1);
    H_mat       = matriz(h_buff, Nc, Nc);
    A_cons      = matriz(a_cons_buff, 4 * Nc, Nc);
    B_cons      = matriz(b_cons_buff, 4 * Nc, 1);
    Delta_U_opt = matriz(du_opt_buff, Nc, 1);
    
    // Usa as matrizes declaradas no topo para respeitar C89
    Phi_T     = matriz(phi_t_buff, Nc, Np);
    Phi_T_Phi = matriz(phi_t_phi_buff, Nc, Nc);
    
    transposta(&Phi, &Phi_T);
    mat_mult(&Phi_T, &Phi, &Phi_T_Phi);
    sm(&Phi_T_Phi, &R_barra, &H_mat);

    // PRÉ-CÁLCULO DO GRADIENTE (Para uso ultrarrápido no SimulationStep)
    Phi_T_F  = matriz(phi_t_f_buff, Nc, n_estados);
    Phi_T_Rs = matriz(phi_t_rs_buff, Nc, 1);
    mat_mult(&Phi_T, &F, &Phi_T_F);
    mat_mult(&Phi_T, &Rs_barra, &Phi_T_Rs);

    // Montagem offline da Matriz de Inequações A_cons
    for(i = 0; i < Nc; i++) {
        for(j = 0; j < Nc; j++) {
            I_val = (i == j) ? 1.0 : 0.0;
            C2_val = (i >= j) ? 1.0 : 0.0;
            A_cons.matriz[(0 * Nc + i) * Nc + j] = I_val;
            A_cons.matriz[(1 * Nc + i) * Nc + j] = -I_val;
            A_cons.matriz[(2 * Nc + i) * Nc + j] = C2_val;
            A_cons.matriz[(3 * Nc + i) * Nc + j] = -C2_val;
        }
    }
    
    // Ganhos ótimos clássicos (Mantidos para visualização e documentação)
    K_mpc = matriz(k_mpc_array, 1, A.linhas);
    func_Kmpc_Ky(&Phi, &R_barra, &Rs_barra, &F, &K_mpc, &K_y);
}
void SimulationEnd(const char *szId, void ** reserved_UserData, int reserved_ThreadIndex, void * reserved_AppPtr)
